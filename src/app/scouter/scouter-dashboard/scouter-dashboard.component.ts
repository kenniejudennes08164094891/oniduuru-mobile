...
export class ScouterDashboardComponent implements OnInit {
  ...
  private updateDashboardWithRealData(stats: any): void {
    ...
    // Update total expenses if needed
    if (stats.totalExpensesThisMonth) {
      // You can handle total expenses here if needed
    }
  ...
  private updateRecentHiresComponent(recentHires: any[]): void {
    ...
    const transformedHires = recentHires.map(hire => {
      ...
      if (hire.dateOfHire) {
        try {
              ...
              if (dateParts.length === 3) {
                ...
                if (timePart) {
                  ...
                  if (time.includes('am') || time.includes('pm')) {
                    time = time.replace('am', ' AM').replace('pm', ' PM').toUpperCase();
                  }
                  formattedTime = time;
                  dateTime = `${formattedDate} at ${formattedTime}`;
                }
          ...
          console.error('Error parsing date/time:', e, 'Date string:', hire.dateOfHire);
      ...
      if (hire.amountToPay) {
        try {
          ...
          console.error('Error parsing amount:', e, 'Amount string:', hire.amountToPay);
          amount = 0;
        }
      }

      // FIX: Properly extract and format talent name
      const talentName = this.extractTalentName(hire);
      console.log('🔍 Extracted talent name:', talentName, 'from hire:', hire);
      ...
      const transformedHire = {
        ...
        dateTime: dateTime,
        amount: amount,
        offerStatus: this.mapHireStatus(hire.hireStatus || 'awaiting-acceptance'),
    ...
    console.log('✅ Final transformed hires:', transformedHires);
    this.RecentHires = transformedHires;
  }

  // Add this new method to properly extract talent name
  private extractTalentName(hire: any): string {
    if (!hire) return 'Unknown Talent';

    console.log('🔍 Extracting talent name from hire data:', hire);

    // Check multiple possible locations for talent name
    // Try direct properties first
    if (hire.talentName) {
      return hire.talentName;
    }

    // Try nested talent object
    if (hire.talent) {
      ...
      // Try email as last resort
      if (talent.email) {
        const emailUsername = talent.email.split('@')[0];
        return emailUsername.charAt(0).toUpperCase() + emailUsername.slice(1);
      }
    }

    // Try user object if exists
    if (hire.user) {
    ...
    console.warn('⚠️ Could not find talent name in hire data. Available keys:', Object.keys(hire));

    // Log the full hire object for debugging
    console.log('🔍 Full hire object for debugging:', JSON.stringify(hire, null, 2));
  ...
  initializeDashboardData() {
    ...
    this.dashboardStatCardsUnpaid = [
      {
        ...
        value: unpaidStatusTotal > 0 ? Math.round((unpaidAccepted / unpaidStatusTotal) * 100) : 0,
        status: 'active',
      },
      {
        title: 'Awaiting Acceptance',
        value: unpaidStatusTotal > 0 ? Math.round((unpaidAwaiting / unpaidStatusTotal) * 100) : 0,
        status: 'pending',
      },
      {
        title: 'Offer Rejected',
        value: unpaidStatusTotal > 0 ? Math.round((unpaidRejected / unpaidStatusTotal) * 100) : 0,
        status: 'inactive',
      },
    ];
    ...
    this.dashboardStatCardsPaid = [
      {
        title: 'Offer Accepted',
        value: paidStatusTotal > 0 ? Math.round((paidAccepted / paidStatusTotal) * 100) : 0,
        status: 'active',
      },
      {
        title: 'Awaiting Acceptance',
        value: paidStatusTotal > 0 ? Math.round((paidAwaiting / paidStatusTotal) * 100) : 0,
        status: 'pending',
      },
      {
        title: 'Offer Rejected',
        value: paidStatusTotal > 0 ? Math.round((paidRejected / paidStatusTotal) * 100) : 0,
        status: 'inactive',
      },
    ];

    console.log('📈 Calculated Percentages - Unpaid:', this.dashboardStatCardsUnpaid);
    console.log('📈 Calculated Percentages - Paid:', this.dashboardStatCardsPaid);

    // Check if percentages add up (for debugging)
    const unpaidPercentTotal = this.dashboardStatCardsUnpaid.reduce((sum, card) => sum + card.value, 0);
    const paidPercentTotal = this.dashboardStatCardsPaid.reduce((sum, card) => sum + card.value, 0);
    console.log('🔢 Percentage Totals - Unpaid:', unpaidPercentTotal + '%', 'Paid:', paidPercentTotal + '%');
  ...
  getPercentageForStatus(value: number, statusType: string): number {
    ...
    console.log(`📐 Percentage calculation: ${value} / ${total} = ${percentage}%`);
    return percentage;
  }
