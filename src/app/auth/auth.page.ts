...
export class AuthPage implements OnInit {
 ...
 async fetchProfilePicture(role:string, userData:any):Promise<any>{
    try{
      const uniqueId = role === 'scouter' ? userData.scouterId : userData.talentId;
      const handleProfilePicture = role === 'scouter' ? await firstValueFrom(this.endpointService.getScouterPicture(uniqueId)) : await firstValueFrom(this.endpointService.getTalentPicture(uniqueId));
      if(handleProfilePicture?.data?.base64Picture){
        localStorage.setItem('profilePicture', handleProfilePicture?.data?.base64Picture);
      }
    }catch (e:any) {
      console.error('an error occurred while fetching your profile picture>>', e);
    }
  }
  ...
  async submitForm(): Promise<void> {
    ...
    try {
      ...
      if (res?.access_token) {
        ...
        await this.fetchProfilePicture(role, res?.details?.user)
