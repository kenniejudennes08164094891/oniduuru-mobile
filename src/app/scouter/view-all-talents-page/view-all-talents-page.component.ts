...
export class ViewAllTalentsPageComponent implements OnInit, OnDestroy {
  ...
  loadCurrentUserData() {
    try {
      const localStorageKeys = ['user_profile_data', 'user_data', 'auth_user', 'current_user'];
      for (const key of localStorageKeys) {
        const data = localStorage.getItem(key);
        if (data) {
          try {
            const parsedData = JSON.parse(data);
            const userName = parsedData.fullName || parsedData.name || parsedData.username || parsedData.firstName || 'Scouter';
            const userLocation = parsedData.location || parsedData.address || parsedData.city || 'Unknown Location';
...
loadTalents() {
  ...
  const subscription = this.scouterService.fetchAllTalents({
    ...
    next: (response) => {
      ...
      this.totalTalents = response.pagination?.totalItems || 
...
private extractTalentsFromResponse(response: any): any[] {
  ...
  if (response.decodedData?.mappedTalents && Array.isArray(response.decodedData.mappedTalents)) {
    console.log('✅ Using decodedData.mappedTalents array');
    return response.decodedData.mappedTalents.map((talent: any) => this.mapTalent(talent));
  }
...
private mapTalent(talent: any): any {
  ...
  let payRange = talent.payRange || 
                (talent.skillLevel ? `Based on ${talent.skillLevel} level` : 'Negotiable');
  ...
  return {
    ...
    profilePic: talent.talentPicture || talent.profilePicture || 'assets/images/default-avatar.png',
